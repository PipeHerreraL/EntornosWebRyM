...
const CharacterDetail = () => {
    ...
    if (!character) {
        return (
            <div className="min-h-screen flex items-center justify-center text-green-900 dark:bg-gray-900 dark:text-white">
    ...
    return (
        <div className="text-white p-4 sm:p-6">
            <button
                onClick={() => navigate(-1)}
                className="mb-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded transition duration-200 max-sm:px-2 max-sm:py-1 max-sm:text-sm max-sm:rounded-md"
            >
                &larr; Volver
            </button>

            <div className="flex flex-col md:flex-row bg-gradient-to-r from-emerald-50 from-10% to-green-50 dark:from-emerald-900 dark:to-green-900 rounded-lg shadow-lg overflow-hidden">
                <img
                    src={character.image}
                    alt={character.name}
                    className="w-full h-60 md:h-auto md:w-64 object-cover"
                />
                <div className="p-4 sm:p-6 flex flex-col justify-between text-sm sm:text-base text-green-900 dark:text-white">
